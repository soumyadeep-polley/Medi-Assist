...
def get_predicted_value(patient_symptoms):
    ...
    return diseases_list[pred], confidence
...
def helper(dis):
    ...
    pre = pre[0] if len(pre)>0 else ["No precautions"]

    med = medications[medications['Disease'] == dis]['Medication'].values
    med = list(med) if len(med)>0 else ["No medication"]

    die = diets[diets['Disease'] == dis]['Diet'].values
    die = list(die) if len(die)>0 else ["No diet"]

    wrk = workout[workout['disease'] == dis]['workout'].values
    wrk = list(wrk) if len(wrk)>0 else ["No workout"]
...
def predict():
    try:
        ...
        disease, confidence = get_predicted_value(symptoms)
        disease_clean = disease.strip().lower()
        desc, pre, med, die, wrk = helper(disease_clean)
